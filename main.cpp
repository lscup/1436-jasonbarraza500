...
int main() {
    ...
    while (score_input) {
        ...
        if (assignment_score >= 0) {
            total_points_earned += assignment_score; // Add to the running total
            assignment++; // Move to the next assignment
        } else {
            score_input = false; // Stop the loop if a negative number is entered
        }
    }
